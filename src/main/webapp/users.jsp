...
<section>
    <jsp:useBean id="users" scope="request" type="java.util.List<ru.javawebinar.topjava.model.User>"/>
                        ...
                        <td colspan="2">
                            <table class="table table-striped display dataTable no-footer">
                                <tr>
                                    ...
                                    <th width="20%">Calories per day</th>
                                    <th width="20%">Edit</th>
                                    ...
                                    <c:forEach items="${users}" var="user">
                                        <c:if test="${user.id > 0}">
                                            <tr class="${user.enabled ? "success" : "danger"}">
                                                ...
                                                <td>${user.email}</td>
                                                <td>${user.password}</td>
                                                <td>${fn:replace(user.registered, 'T', ' ').replaceAll(":\\d{2}.\\d{6}", "")}</td>
                                                <td>${user.enabled ? "включён" : "отключён"}</td>
                                                <td>${user.caloriesPerDay}</td>
                                                <td><a href="<c:url value='?action=update&id=${user.id}'/>"
                                                       class="btn btn-primary btn-xs"
                                                       role="button">Update</a>
                                                <td><a href="<c:url value='?action=delete&id=${user.id}'/>"
                                                       target="_self"
