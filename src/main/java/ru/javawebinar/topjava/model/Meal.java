...
@NamedQueries({
        ...
        @NamedQuery(name = Meal.BETWEEN_SORTED, query = "SELECT m FROM Meal m LEFT JOIN FETCH m.user WHERE m.user.id=:userid AND m.dateTime BETWEEN :startDate AND :endDate ORDER BY m.dateTime DESC"),
})
